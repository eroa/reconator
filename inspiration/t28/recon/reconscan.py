...
def httpEnum(ip_address, port):
    ...
    HTTPSCAN = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-email-harvest,http-methods,http-method-tamper,http-passwd,http-robots.txt -oN /home/auh/olab_proj/tools/recon/results/%s_http.nmap %s" % (port, ip_address, ip_address)
...
def httpsEnum(ip_address, port):
    ...
    print "INFO: Performing nmap web script scan for " + ip_address + ":" + port
    HTTPSCANS = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-email-harvest,http-methods,http-method-tamper,http-passwd,http-robots.txt -oX /home/auh/olab_proj/tools/recon/results/%s_https.nmap %s" % (port, ip_address, ip_address)
...
def nmapScan(ip_address):
   ...
   serv_dict = {}
   TCPSCAN = "nmap -vv -Pn -A -sC -sS -T 4 -p- -oN '/home/auh/olab_proj/tools/recon/results/%s.nmap' -oX '/home/auh/olab_proj/tools/recon/results/nmap/%s_nmap_scan_import.xml' %s"  % (ip_address, ip_address, ip_address)
   UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN '/home/auh/olab_proj/tools/recon/results/%sU.nmap' -oX '/home/auh/olab_proj/tools/recon/results/nmap/%sU_nmap_scan_import.xml' %s" % (ip_address, ip_address, ip_address)
...
if __name__=='__main__':
   f = open('results/targets.txt', 'r') # CHANGE THIS!! grab the alive hosts from the discovery scan for enum
   for scanip in f:
       jobs = []
